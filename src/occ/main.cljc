(ns occ.main
  "Kotodama WASM entrypoint for the Occ clean-room actor (L4) — Clojure port.
  ...
  facts (`occ.<Entity>/<field>`); `*store*` is the in-memory materialization
...
(def entity-specs
  [{:entity "Instrument" :plural "instruments" :id-prefix "occ_ins"
   ...
   {:entity "Position" :plural "positions" :id-prefix "occ_pos"
...
(defn emit-facts
  "EAVT facts for one record: {\"occ.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "occ-compat" :tier tier :entities entities} 200])
